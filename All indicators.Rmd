...
house_values_data <- Quandl.datatable("ZILLOW/DATA", indicator_id="ZALL", region_id="102001") 
house_values <- xts(x = house_values_data, 
                    order.by= house_values_data$date)
house_values <- house_values [,4]

house_sales <- Quandl.datatable("ZILLOW/DATA", indicator_id="SAAW", region_id="102001") #**Not included in analysis**

chartSeries(house_values)
...
retail_sales <- Quandl("FRED/RSXFS",start_date=SDate, end_date=EDate ,type="xts", collapse = "monthly")
...
## Real GDP growth - USGdpRealReturn **CAREFUL WITH MONTHS**
```{r}
...
joined %>% 
  mutate(Value = na.approx(Value)) #ne dela
...
UnitLaborCosts <- Quandl('FRED/PRS30006111', start_date=SDate, end_date=EDate, type = "xts", collapse = "monthly") #**CONVERT**
...
plot(x= CPI_CORE$Date, y=CPI_CORE$yearOverYear)
plot(x= CPI$Date, y=CPI$yearOverYear)
...
plot(x= cpi_spread$Date, y=cpi_spread$spread)
...
```{r}
my_values <- c(4, 9, 10, NA, NA, 5, 12, NA, 7, 11, 8) #test
my_df <- data.frame(my_values)

my_df %>% 
  mutate(my_values = na.approx(my_values))
```
