...
stockmarket_xts <- xts(x = stockmarket$adjusted, 
                    order.by= stockmarket$date)
...
stockmarket_returns_xts <- xts(x = stockmarket_returns$SP500_returns, 
                    order.by= stockmarket_returns$date)

stockmarket_returns_xts %>%
  as.data.frame() %>%
  rownames_to_column("date") %>%
  ggplot(aes(x=as.Date(date), y=V1)) +
  geom_line() +
  theme_classic() +
