...
## Stock market ##TODO convert to monthly
```{r}
# S&P500 Prices
...
## All Employees, Retail Trade ##TODO WRONG INDICATOR
```{r}
...
## Building permits ##TODO check if we can get more recent data
```{r}
...
## Average Weekly Hours of Production and Nonsupervisory Employees, Manufacturing ##TODO check more frequent data
```{r}
...
avg_weekly_hours_prod <- tq_get("AWHMAN", 
  ...
  Quandl("FRED/AWHMAN", start_date=SDate, end_date=EDate, collapse = "monthly") %>% 
  rename(AvgWeeklyHoursProd = Value) %>% 
  mutate(Date = floor_date(Date, unit = "month"))
```

## Future New Orders; Diffusion Index for Federal Reserve District 3: Philadelphia ##TODO check more frequent data
```{r}
...
future_new_orders <- tq_get("NOFDFSA066MSFRBPHI", 
  ...
  Quandl("FRED/NOFDFSA066MSFRBPHI", start_date=SDate, end_date=EDate, collapse = "monthly") %>% 
...
## Industrial production ##TODO check more frequent data
```{r}
...
ind_prod_ind <- tq_get("INDPRO", 
  ...
  Quandl("FRED/INDPRO", start_date=SDate, end_date=EDate, collapse = "monthly") %>% 
...
disp_pers_income <- tq_get("DSPIC96", 
  ...
  Quandl("FRED/DSPIC96", start_date=SFDate, end_date=EFDate, collapse = "monthly") %>% 
...
## Consumer debt to income ##TODO fix interpolation
```{r}
